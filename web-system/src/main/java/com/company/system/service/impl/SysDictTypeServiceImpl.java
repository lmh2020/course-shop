...
import com.company.common.utils.DictUtils;
...
//        }
     ...
     */
    @Override
    public Map<String, List<SysDictData>> selectDictDataByMultiType(String dictTypes) {
        AssertUtil.isBlank(dictTypes, "字典值不能为空");
        String[] dictTypeArray = dictTypes.split(",");
        Map<String, List<SysDictData>> multiDicts = new HashMap<>(dictTypeArray.length);
        for (String dictType : dictTypeArray) {
            multiDicts.put(dictType, this.selectDictDataByType(dictType));
        }
        return multiDicts;
    }
    ...
    @Override
    public Boolean checkDictValueExist(String dictType, String dictValue) {
        return this.baseMapper.checkDictValueExist(dictType, dictValue);
    }
