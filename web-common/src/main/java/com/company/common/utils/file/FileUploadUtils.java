package com.company.common.utils.file;
...
import com.company.common.utils.uuid.IdUtils;
...
public class FileUploadUtils {
    ...
    public static final String upload(String baseDir, MultipartFile file, String[] allowedExtension) throws IOException {
        ...
        if (fileNamelength > FileUploadUtils.DEFAULT_FILE_NAME_LENGTH) {
            throw new SystemException("文件名长度超出最大限制：" + FileUploadUtils.DEFAULT_FILE_NAME_LENGTH);
        }
    ...
    public static final void assertAllowed(MultipartFile file, String[] allowedExtension) {
        ...
        if (DEFAULT_MAX_SIZE != -1 && size > DEFAULT_MAX_SIZE) {
            throw new SystemException("文件大小超过系统限制：" + DEFAULT_MAX_SIZE / 1024 / 1024 + "M");
        }
        if (allowedExtension != null && !isAllowedExtension(getExtension(file), allowedExtension)) {
            if (allowedExtension == MimeTypeUtils.IMAGE_EXTENSION) {
                throw new SystemException("允许上传的文件类型为：" + JSON.toJSONString(allowedExtension));
            }
        }
    }
